...
Use the machine-global `grok-worker` command as the only production path. It is a stable shim at `%USERPROFILE%\.localin\grok-worker.cmd`, resolved through `%LOCALAPPDATA%\GrokWorkerProvider\current.json` to an immutable release. Install it with `grok-worker deploy pointer` from a checkout of this repository; that checkout is the authoritative source. Never call a provider copy that lives inside another tool's runtime directory as a caller, and never edit such a copy in place.
...
1. **Is the Grok CLI installed?** `grok --version` must succeed.
   `grok-worker doctor` does **not** check this -- it only validates this
   provider's own directory layout and will report `pass: true` on a machine
   with no Grok CLI at all. The failure would otherwise surface much later,
   inside `onboard` or `run`, as a low-level error that does not name the
   real cause.
