  ...
  for (const taskDir of fs.readdirSync(root, { withFileTypes: true })) {
    ...
    for (const name of fs.readdirSync(dir).filter((n) => n.endsWith(".json"))) {
      ...
      if (run && run.status === "running") {
        ...
        const originalStartup = run.startupHandshake || null;
        let childInspection = null;
        if (originalStartup && originalStartup.childPid !== null) {
          childInspection = typeof deps.inspectRunOwner === "function"
            ? deps.inspectRunOwner({
              pid: originalStartup.childPid,
              processStartTicks: originalStartup.childStartTicks,
              capturedAt: originalStartup.osSpawnedAt || originalStartup.startedAt
            })
            : { state: "unverifiable", reason: "child-inspector-missing" };
          // The Provider owner may die while the exact child still has write
          // authority. Keep WAL recovery blocked until that child is proven gone.
          if (!childInspection || childInspection.state !== "dead") continue;
        } else if (originalStartup && [
          "os_spawned", "socket_observed", "first_stdout_byte", "first_valid_event"
        ].includes(originalStartup.phase)) {
          // A post-spawn record without an exact child identity cannot safely
          // release custody during owner recovery.
          continue;
        }
        try {
          let latest;
          ...
          if (originalStartup) {
            const latestStartup = latest.startupHandshake || null;
            const sameChild = latestStartup
              && latestStartup.invocationId === originalStartup.invocationId
              && latestStartup.childPid === originalStartup.childPid
              && latestStartup.childStartTicks === originalStartup.childStartTicks;
          ...
          if (originalStartup && originalStartup.childPid !== null
              && typeof deps.reconcileStartupHandshake === "function") {
            const custody = deps.reconcileStartupHandshake(clone(latest.startupHandshake), childInspection);
            if (!custody || custody.childDead !== true) continue;
            latest.startupHandshake = {
              ...latest.startupHandshake,
              phase: "startup_failed",
              termination: "exited",
              cleanup: custody.cleanup === "complete" ? "complete" : "failed",
              failureReason: latest.startupHandshake.failureReason || custody.reason || "provider-owner-recovered-after-child-close",
              lastProgressAt: custody.reconciledAt || latest.startupHandshake.lastProgressAt
            };
          }
