...
function taskRunLockPath(dataRoot, taskId, runId) {
  return `${runWalPath(dataRoot, taskId, runId)}.lock`;
}
...
function acquireTaskRunLock(dataRoot, taskId, runId, deps) {
  const file = taskRunLockPath(dataRoot, taskId, runId);
  deps.ensureDir(path.dirname(file));
  const owner = typeof deps.captureRunOwner === "function" ? deps.captureRunOwner() : null;
  if (!owner) return null;
  const lock = { lockId: crypto.randomUUID(), owner, acquiredAt: nowIso() };
  let fd;
  try {
    ...
    return {
      ...
      release() {
        let current = null;
        try { current = deps.readJson(file); } catch (_) { /* fail closed below */ }
        if (current && current.lockId === lock.lockId) fs.rmSync(file, { force: true });
      }
    };
  } catch (error) {
    if (fd !== undefined) {
    ...
    // Never delete/reclaim a lock path automatically: another process could
    // replace a stale lock between inspection and unlink. Fail closed and leave
    // stale-lock reconciliation to an explicit controller action.
    return null;
  }
}

function writeTaskRunLocked(file, taskRun, deps) {
  ...
  let current = null;
  if (fs.existsSync(file)) current = deps.readJson(file);
  const actualRevision = current ? Number(current.revision) : 0;
  if ((current && !Number.isInteger(actualRevision)) || actualRevision !== expectedRevision) {
    throw taskRunError("TASK_RUN_CAS_CONFLICT", "Task-run revision changed before commit.");
  }
  if (current && ["completed", "failed", "interrupted"].includes(current.status)
      && taskRun.status !== current.status) {
    throw taskRunError("TASK_RUN_TERMINAL_CONFLICT", "A terminal task-run state is immutable.");
  }
...
function writeTaskRun(dataRoot, taskRun, deps) {
  ...
  const held = acquireTaskRunLock(dataRoot, taskRun.taskId, taskRun.runId, deps);
  if (!held) throw taskRunError("TASK_RUN_LOCKED", "Task-run WAL is held by another live writer.");
  try {
    return writeTaskRunLocked(file, taskRun, deps);
  } finally {
    held.release();
  }
}
  ...
  for (const taskDir of fs.readdirSync(root, { withFileTypes: true })) {
    ...
    for (const name of fs.readdirSync(dir).filter((n) => n.endsWith(".json"))) {
      ...
      if (run && run.status === "running") {
        ...
        const held = acquireTaskRunLock(dataRoot, run.taskId, run.runId, deps);
        if (!held) continue;
        try {
          let latest;
          try {
          ...
          const written = writeTaskRunLocked(file, latest, deps);
          recovered.push({
