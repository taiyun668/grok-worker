...
function writeMaintenanceProfileCas(dataRoot, profileId, nextRecord, expectedRevision, deps) {
  const file = maintenanceProfilePath(dataRoot, profileId);
  deps.ensureDir(path.dirname(file));
  let currentRevision = 0;
  if (fs.existsSync(file)) {
...
function threeStateFilePlan(current, before, target) {
  ...
  if (isObject(current) && isObject(before) && isObject(target)) {
    if (Number(current.revision) === Number(target.revision)
      && current.state === target.state
      && current.episodeId === target.episodeId) {
      return { action: "done" };
    }
    if (Number(current.revision) === Number(before.revision)
      && (before.state == null || current.state === before.state)
      && (before.episodeId == null || current.episodeId === before.episodeId)) {
    ...
    for (const name of fs.readdirSync(dir).filter((n) => n.endsWith(".json"))) {
      ...
      if (availPlan.action === "forward" && journal.availabilityTarget) {
        const expected = journal.availabilityBefore && Number.isFinite(Number(journal.availabilityBefore.revision))
          ...
          : 0;
        const cas = writeAvailabilityCas(dataRoot, profileId, journal.availabilityTarget, expected, deps);
        if (!cas.ok) {
          journal.status = "interrupted";
          journal.updatedAt = nowIso();
          deps.atomicWriteJson(file, journal);
      ...
      if (sidePlan.action === "forward" && journal.sidecarTarget) {
        const expected = journal.sidecarBefore && Number.isFinite(Number(journal.sidecarBefore.revision))
          ...
          : 0;
        const cas = writeMaintenanceProfileCas(dataRoot, profileId, journal.sidecarTarget, expected, deps);
        if (!cas.ok) {
          journal.status = "interrupted";
          journal.updatedAt = nowIso();
          deps.atomicWriteJson(file, journal);
...
function validateDeployPointerV6(value) {
  if (!isObject(value)) return { ok: false, reason: "not-object" };
  for (const field of ["version", "releasePath", "dataRoot", "registryPath", "manifestSha256", "updatedAt"]) {
