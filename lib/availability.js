  ...
  for (const taskDir of fs.readdirSync(root, { withFileTypes: true })) {
    ...
    for (const name of fs.readdirSync(dir).filter((n) => n.endsWith(".json"))) {
      ...
      if (run && run.status === "running") {
        ...
        // The owner is dead, so it cannot make a later write. Re-read once to
        // avoid overwriting a terminal write that landed before process exit.
        let latest;
        try {
        ...
        deps.atomicWriteJson(file, latest);
        recovered.push({
