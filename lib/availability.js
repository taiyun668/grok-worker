...
function acquireTaskRunLock(dataRoot, taskId, runId, deps) {
  ...
  for (let attempt = 0; attempt < 2; attempt += 1) {
    ...
    try {
      ...
      let existing = null;
      try { existing = deps.readJson(file); } catch (_) { return null; }
      const inspection = typeof deps.inspectRunOwner === "function"
        ? deps.inspectRunOwner(existing && existing.owner)
        : { state: "unverifiable" };
      if (!inspection || inspection.state !== "dead") return null;
      try { fs.rmSync(file, { force: false }); } catch (_) { return null; }
    }
  }
  return null;
}
