...
function createStreamAccumulator(onValidEvent) {
  ...
  function consumeLine(line) {
    ...
    if (type === "text" && typeof event.data === "string") parsed.finalText += event.data;
    parsed.summary.push({
      type,
...
function executePlanAsync(plan, options = {}) {
  ...
  return new Promise((resolve, reject) => {
    ...
    const acc = createStreamAccumulator((event) => {
      ...
      } else if (progressState.requestObservation !== "observed"
    ...
    child.once("close", (code, signal) => {
      ...
      if (!progressState.firstValidEventAt && !failureClass) {
        ...
        failureReason = reasonBeforeFirstEvent();
...
function requestObservationForExecution(execution) {
  ...
  const summary = execution && execution.parsed && execution.parsed.summary || [];
  return summary.some((event) => typeof event.requestId === "string" && event.requestId.length > 0)
    ? "observed"
    : "unknown";
}
      ...
      if (error && error.childClosed === false) {
        throw new ProviderError(
          ...
          "The exact worker child did not confirm termination after a Provider progress failure; invocation files are retained and automatic retry is forbidden.",
          {
            ...
            requestObservation: startupHandshake && startupHandshake.requestObservation || "unknown",
            realRequests: startupHandshake && startupHandshake.requestObservation === "observed" ? 1 : "unknown",
            startupHandshake
          }
        );
      }
    ...
    if (!execution.childClosed) {
      throw new ProviderError(
        ...
        "The exact worker child did not confirm termination; invocation files are retained and automatic retry is forbidden.",
        {
          ...
          requestObservation: startupHandshake.requestObservation,
          realRequests: startupHandshake.requestObservation === "observed" ? 1 : "unknown",
          startupHandshake
        }
      );
    }
...
function runTask(alias, taskFile, options = {}) {
  return driveMaybeAsync(runTaskFlow(alias, taskFile, options));
}
  ...
  try {
    ...
    if (error && error.name === "ProviderError") {
      process.stdout.write(`${JSON.stringify({
        ...
        realRequests: error.details && Object.prototype.hasOwnProperty.call(error.details, "realRequests")
          ? error.details.realRequests
          : 0
      }, null, 2)}\n`);
