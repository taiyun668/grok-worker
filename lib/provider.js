...
function verifyMaintenancePlanContract(plan, profile) {
  ...
  assert(profile && typeof profile.executable === "string" && profile.executable, "MAINTENANCE_PLAN_CONTRACT", "Maintenance profile has no verified CLI.");
  assert(plan.executable === profile.executable, "MAINTENANCE_PLAN_CONTRACT", "Maintenance executable is not the selected profile CLI.");
  ...
  requireValue("--output-format", "streaming-json"); requireValue("--model", "grok-4.5"); requireValue("--reasoning-effort", "high"); requireValue("--max-turns", "1"); requireValue("--cwd", scratch);
  ...
  assert(env.GROK_HOME === profile.grokHome && env.GROK_HOME !== process.env.GROK_HOME, "MAINTENANCE_PLAN_CONTRACT", "Maintenance GROK_HOME is not the isolated profile home.");
