...
const STREAM_EVENT_FIELDS = Object.freeze({
  text: new Set(["type", "data"]),
  end: new Set(["type", "sessionId", "requestId", "usage", "stopReason", "rate_limits", "quota"])
});
const STREAM_USAGE_FIELDS = new Set([
  "input_tokens", "cache_read_input_tokens", "output_tokens", "reasoning_tokens", "total_tokens", "modelUsage"
]);
...
function validUsagePayload(usage) {
  if (!isObject(usage) || Object.keys(usage).length === 0
      || Object.keys(usage).some((key) => !STREAM_USAGE_FIELDS.has(key))) return false;
  for (const [key, value] of Object.entries(usage)) {
    if (key === "modelUsage") {
      if (!isObject(value)) return false;
      for (const modelUsage of Object.values(value)) {
        if (!isObject(modelUsage)
            || Object.keys(modelUsage).some((field) => field !== "modelCalls")
            || typeof modelUsage.modelCalls !== "number"
            || !Number.isFinite(modelUsage.modelCalls)) return false;
      }
...
function validateStreamEvent(event) {
  ...
  if (!Object.prototype.hasOwnProperty.call(STREAM_EVENT_FIELDS, event.type)) {
    return { valid: false };
  }
  const allowedFields = STREAM_EVENT_FIELDS[event.type];
  if (!allowedFields || Object.keys(event).some((key) => !allowedFields.has(key))) return { valid: false };
  if (event.type === "text") {
    return typeof event.data === "string"
      ? { valid: true, type: "text", requestObserved: false, terminalSuccess: false }
      : { valid: false };
  }
