...
function resolveRootsFromPointer() {
  ...
  const source = (envData || envReg || envApproved)
    ? "env"
    : (pointer ? "current.json" : "provider-default");
  return {
...
function buildMaintenanceProbeTargets(profile, record, sidecar, options = {}) {
  ...
  sideTarget.lastMaintenanceProbeAt = new Date(nowMs).toISOString();
  ...
  try {
    ...
    for (const candidate of selection.selected) {
      ...
      try {
        ...
        targets.sidecarTarget.availabilityRevisionSeen = record.revision + 1;
        ...
        const availCas = availability.writeAvailabilityCas(
          DATA_ROOT, profile.profileId, targets.availabilityTarget, record.revision, deps
        );
        if (!availCas.ok) {
        ...
        targets.sidecarTarget.availabilityRevisionSeen = availCas.record.revision;
        targets.sidecarTarget.availabilityStateSeen = availCas.record.state;
        const sideCas = availability.writeMaintenanceProfileCas(
          DATA_ROOT, profile.profileId, targets.sidecarTarget, sidecar.revision, deps
        );
        if (!sideCas.ok) {
          journal.status = "interrupted";
          journal.phase = "availability-committed";
        ...
        } else if (outcome.outcome === "recovered" || (outcome.record && outcome.outcome !== "no-op" && outcome.outcome !== "inconclusive")) {
          availability.writeAvailabilityCas(
            DATA_ROOT, profile.profileId, outcome.record, postRecord.revision, deps
          );
          if (outcome.outcome === "recovered") {
            const sideNow = availability.loadMaintenanceProfile(DATA_ROOT, profile.profileId, deps);
            const cleared = availability.clearFreezeEpisode(sideNow);
            availability.writeMaintenanceProfileCas(
              DATA_ROOT, profile.profileId, cleared, sideNow.revision, deps
            );
          }
        }
