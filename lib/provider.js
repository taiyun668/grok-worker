...
const AVAILABILITY_ROOT = path.join(DATA_ROOT, "availability");
...
function buildResultCapsule({ capsule, plan, execution, classification, selectionEvidence, quotaSignal }) {
  ...
  const changes = changedFilesFinalState(capsule.worktree.path);
...
function persistAvailabilityFromOutcome(profile, classification, executionStatus) {
  ...
  if (touched) {
    ...
    return { cas, reason, record: cas.ok ? cas.record : current };
  }
  return { cas: { ok: true, skipped: true }, reason, record: current };
}
...
function runSingleAttempt(capsule, registry, profile, selectionEvidence, options = {}) {
  ...
  try {
    ...
    persistAvailabilityFromOutcome(profile, classification, execution.status);
  } finally {
  ...
  while (currentProfile && !tried.has(currentProfile.profileId)) {
    ...
    if (!nextId) {
      ...
      if (capsule.failover && Array.isArray(capsule.failover.allowedFallbackProfiles)) {
        ...
        if (!nextAlias) break;
        currentProfile = registry.profiles.find((x) => x.alias === nextAlias);
      } else break;
    } else {
      currentProfile = registry.profiles.find((x) => x.profileId === nextId);
    }
...
function readDeployPointer() {
  try {
    return availability.readCurrentPointer(CURRENT_POINTER_PATH, { readJson });
  } catch (error) {
    return { error: "CURRENT_POINTER_INVALID", path: CURRENT_POINTER_PATH };
  }
}
...
module.exports = {
  ...
  CURRENT_POINTER_PATH, AVAILABILITY_ROOT, RUNS_ROOT,
