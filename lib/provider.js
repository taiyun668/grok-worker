...
const SECRET_VALUE = /(bearer\s+[a-z0-9._-]+|xai-[a-z0-9_-]+)/ig;
const SECRET_HEADER = /(["']?(?:authorization|cookie)["']?\s*[:=]\s*)("[^"]*"|'[^']*'|[^"\r\n,}]+)/ig;
const SECRET_ASSIGNMENT = /(["']?(?:api[_-]?key|access[_-]?token|refresh[_-]?token|password|passphrase|secret)["']?\s*[:=]\s*)("[^"]*"|'[^']*'|[^\s,;]+)/ig;
const MAX_WORKER_SUMMARY_BYTES = 16 * 1024;
const STARTUP_HANDSHAKE_TIMEOUT_MS = 30_000;
const CHILD_TERMINATION_CONFIRM_TIMEOUT_MS = 5_000;
...
function availabilityDeps() {
  return {
    ...
    captureRunOwner,
    inspectRunOwner,
    reconcileStartupHandshake,
    taskRunTransaction
  };
}

function queryProcessStartTicks(pid, timeoutMs = 5000) {
...
function redactText(value) {
  return String(value || "")
    ...
    .replace(/[A-Za-z0-9+/]{48,}={0,2}/g, "[REDACTED_BLOB]")
    .replace(SECRET_ASSIGNMENT, "$1[REDACTED]");
}

function reconcileStartupHandshake(handshake, childInspection) {
  if (!isObject(handshake) || !UUID.test(String(handshake.invocationId || ""))) {
    return { childDead: false, cleanup: "retained", reason: "startup-identity-missing" };
  }
  const childKnownAbsent = handshake.childPid === null
    && ["OS_SPAWN_FAILED", "PROVIDER_PRESPAWN_FAILED"].includes(handshake.failureClass);
  const childNeverSpawned = handshake.childPid === null
    && handshake.phase === "materializing"
    && handshake.termination === "not_started";
  const childDead = childKnownAbsent || childNeverSpawned
    || Boolean(childInspection && childInspection.state === "dead");
  if (!childDead) return { childDead: false, cleanup: "retained", reason: "worker-child-not-proven-dead" };

  const expectedRoot = path.resolve(TEMP_ROOT, handshake.invocationId);
  const expectedSocket = path.resolve(expectedRoot, "leader.sock");
  if (path.resolve(handshake.socketPath || "") !== expectedSocket) {
    return { childDead: true, cleanup: "failed", reason: "invocation-socket-path-mismatch", reconciledAt: now() };
  }
  try {
    fs.rmSync(expectedRoot, { recursive: true, force: true });
    return { childDead: true, cleanup: "complete", reason: "provider-owner-and-child-dead", reconciledAt: now() };
  } catch (_) {
    return { childDead: true, cleanup: "failed", reason: "invocation-cleanup-failed", reconciledAt: now() };
  }
}
function sanitizeErrorDetails(value, depth = 0) {
  if (depth > 8) return "[TRUNCATED]";
  if (Array.isArray(value)) return value.slice(0, 64).map((item) => sanitizeErrorDetails(item, depth + 1));
  if (!isObject(value)) return typeof value === "string" ? redactText(value).slice(0, 4096) : value;
  const safe = {};
  for (const [key, item] of Object.entries(value)) {
    if (/(?:stderr|stdout|prompt|auth|token|secret|password|cookie|api.?key|raw)/i.test(key)) continue;
    safe[key] = sanitizeErrorDetails(item, depth + 1);
  }
  return safe;
}
function boundedWorkerSummary(value) {
  const redacted = redactText(value).trim();
  const payload = Buffer.from(redacted, "utf8");
  if (payload.length <= MAX_WORKER_SUMMARY_BYTES) return redacted;
  return `${payload.subarray(0, MAX_WORKER_SUMMARY_BYTES).toString("utf8").replace(/\uFFFD$/, "")}\n[TRUNCATED]`;
}
...
function projectSettingsHasAuthority(text) {
  let parsed;
  try { parsed = JSON.parse(text); } catch (_) { return true; }
  if (!isObject(parsed)) return true;
  return Object.keys(parsed).some((key) => /^(permissions|hooks|mcp|plugin|lsp|sandbox)/i.test(key));
}
function preflightProject(worktree) {
  const findings = [];
  for (const file of listAuthorityFiles(worktree)) {
    const text = fs.readFileSync(file, "utf8");
    if (/settings(?:\.local)?\.json$/i.test(file)) {
      if (projectSettingsHasAuthority(text)) findings.push({ file, reason: "authority-bearing project configuration" });
      continue;
    }
    if (/\.grok[\\/]hooks/i.test(file) || /mcp|plugin|folder[_-]?trust|\[permission\]|allow\s*=|lsp/i.test(text) || /mcp|lsp/i.test(file)) findings.push({ file, reason: "authority-bearing project configuration" });
  }
...
function permissionSourceFile(source) {
  return String(source || "").replace(/\\/g, "/").replace(/ \([^)]*\)$/, "").toLowerCase();
}
function isProviderInjectedPermissionSource(source, invocationHome) {
  if (!invocationHome) return false;
  const expected = path.join(invocationHome, ".claude", "settings.json").replace(/\\/g, "/").toLowerCase();
  return permissionSourceFile(source) === expected;
}
function inspectAuthorityFindings(parsed, invocationHome) {
  ...
  if (Array.isArray(parsed.hooks)) {
    ...
    if (foreignHooks.length) { suspicious.push("hooks"); details.foreignHooks = foreignHooks; }
  }
  ...
  const unauthorizedSources = permissionSources.filter((source) => !isProviderInjectedPermissionSource(source, invocationHome));
  const loaded = parsed.permissions && Number(parsed.permissions.loaded) || 0;
  // Provider writes isolated HOME/.claude/settings.json (dontAsk). grok inspect
  // reports that file as a permission source with loaded>0. That is authorized.
  // Extra sources, or rules with no source path, are not.
  if (unauthorizedSources.length || (permissionSources.length === 0 && loaded > 0)) {
    suspicious.push("permissionSources");
    details.unauthorizedSources = unauthorizedSources;
  }
...
function inspectEffective(profile, worktree, invocationHome, socket) {
  ...
  const findings = inspectAuthorityFindings(parsed, invocationHome);
  assert(findings.suspicious.length === 0, "INSPECT_AUTHORITY", "Effective config contains unresolved or unauthorized authority.", findings);
...
const STREAM_USAGE_FIELDS = new Set([
  "input_tokens", "cache_read_input_tokens", "cache_creation_input_tokens",
  "output_tokens", "reasoning_tokens", "total_tokens"
]);
const STREAM_QUOTA_FIELDS = new Set(["primary", "used_percent", "usedPercent", "status", "reason"]);

function validNumericObject(value, allowedFields) {
  if (!isObject(value) || Object.keys(value).some((key) => !allowedFields.has(key))) return false;
  for (const [key, item] of Object.entries(value)) {
    if (key === "primary" || key === "modelUsage") continue;
...
function validUsagePayload(usage) {
  if (!isObject(usage) || Object.keys(usage).length === 0) return false;
  for (const [key, value] of Object.entries(usage)) {
    if (STREAM_USAGE_FIELDS.has(key) && (typeof value !== "number" || !Number.isFinite(value))) {
      return false;
    }
  }
  return true;
...
function validateStreamEvent(event) {
  if (!isObject(event) || typeof event.type !== "string") return { valid: false };
  if (event.type !== "end"
      && (Object.prototype.hasOwnProperty.call(event, "sessionId")
        || Object.prototype.hasOwnProperty.call(event, "requestId"))) {
    return { valid: false };
  }
  if (event.type === "text" || event.type === "thought") {
    return typeof event.data === "string"
      ? { valid: true, type: event.type, requestObserved: false, terminalSuccess: false }
      : { valid: false };
  }
  ...
  if (event.type === "usage") {
    return typeof event.messageId === "string" && event.messageId.trim()
      && typeof event.stopReason === "string" && validUsagePayload(event.usage)
      ? { valid: true, type: "usage", requestObserved: false, terminalSuccess: false }
      : { valid: false };
  }
  if (event.type === "plan") {
    return Array.isArray(event.entries)
      ? { valid: true, type: "plan", requestObserved: false, terminalSuccess: false }
      : { valid: false };
  }
  if (event.type === "available_commands") {
    return Array.isArray(event.tools) && Array.isArray(event.commands)
      ? { valid: true, type: "available_commands", requestObserved: false, terminalSuccess: false }
      : { valid: false };
  }
  if (event.type === "error") {
    return typeof event.message === "string"
      ? { valid: true, type: "error", requestObserved: false, terminalSuccess: false }
      : { valid: false };
  }
  if (event.type === "max_turns_reached" || event.type.startsWith("auto_compact_")) {
    return { valid: true, type: event.type, requestObserved: false, terminalSuccess: false };
  }
  if (event.type !== "end") return { valid: false };
  if (typeof event.sessionId !== "string" || !event.sessionId.trim()
      || typeof event.requestId !== "string" || !event.requestId.trim()) return { valid: false };
  if (event.stopReason !== undefined && typeof event.stopReason !== "string") return { valid: false };
  if (event.usage !== undefined && !validUsagePayload(event.usage)) return { valid: false };
  for (const field of ["rate_limits", "quota"]) {
    if (event[field] === undefined) continue;
    const envelope = event[field];
    if (!isObject(envelope)) return { valid: false };
    const direct = { ...envelope };
    const primary = direct.primary;
    delete direct.primary;
    if (!validNumericObject(direct, STREAM_QUOTA_FIELDS)) return { valid: false };
    if (primary !== undefined && !validNumericObject(primary, STREAM_QUOTA_FIELDS)) return { valid: false };
  }
  return {
    valid: true,
    type: "end",
    requestObserved: true,
    terminalSuccess: !/cancel|fail|error|interrupt/i.test(String(event.stopReason || ""))
  };
}

function isSuccessfulTerminalEvent(event) {
  const validation = validateStreamEvent(event);
  return validation.valid && validation.type === "end" && validation.terminalSuccess;
}

function isObservedTerminalEvent(event) {
  const validation = validateStreamEvent(event);
  return validation.valid && validation.type === "end" && validation.requestObserved;
}

function isVerifiedSuccessfulResult(result) {
  return Boolean(result
    && result.status === "completed"
    && result.errorClassification
    && result.errorClassification.errorType === null
    && result.errorClassification.note === "no-error-on-success"
    && result.boundaryCompliance
    && result.boundaryCompliance.allowed === true
    && result.redaction
    && result.redaction.rawStreamDeleted === true
    && result.redaction.rawCleanupFailed === false
    && typeof result.grokSessionId === "string"
    && result.grokSessionId.length > 0
    && typeof result.requestId === "string"
    && result.requestId.length > 0
    && !result.requestId.startsWith("unknown-")
    && Array.isArray(result.findings)
    && result.findings.includes("terminal end event observed"));
}
...
function parseStream(text) {
  ...
  for (const line of String(text || "").split(/\r?\n/).filter(Boolean)) {
    ...
    const validation = validateStreamEvent(event);
    if (!validation.valid) { invalid += 1; continue; }
    if (validation.type === "text") finalText += event.data;
    summary.push({ type: validation.type, sessionId: event.sessionId || null, requestId: event.requestId || null, hasUsage: Boolean(event.usage), textBytes: typeof event.data === "string" ? Buffer.byteLength(event.data) : 0 });
    if (validation.type === "end") terminal = event;
  }
...
function pathContentFingerprint(target) {
  let stat;
  ...
  if (stat.isDirectory()) {
    ...
    for (const name of fs.readdirSync(target).sort()) {
      ...
      hash.update(pathContentFingerprint(path.join(target, name))); hash.update("\0");
    }
    return `dir:${hash.digest("hex")}`;
  }
  return `other:${stat.mode}:${stat.size}:${stat.mtimeMs}:${stat.ctimeMs}`;
}
function changedFilesSnapshot(worktree) {
  return changedFilesFinalState(worktree).map((entry) => ({
    ...entry,
    fingerprint: pathContentFingerprint(path.join(worktree, entry.path.includes(" -> ")
      ? entry.path.split(" -> ").pop()
      : entry.path))
  }));
}
function changedFilesDelta(before, after) {
  const initial = new Map((before || []).map((entry) => [entry.path, entry]));
  const final = new Map((after || []).map((entry) => [entry.path, entry]));
  const changed = [];
  for (const name of new Set([...initial.keys(), ...final.keys()])) {
    const oldEntry = initial.get(name); const newEntry = final.get(name);
    if (!oldEntry) {
      changed.push({ code: newEntry.code, path: newEntry.path, ignored: newEntry.ignored });
    } else if (!newEntry) {
      changed.push({ code: "D ", path: oldEntry.path, ignored: oldEntry.ignored, baselineRemoved: true });
    } else if (oldEntry.code !== newEntry.code || oldEntry.ignored !== newEntry.ignored || oldEntry.fingerprint !== newEntry.fingerprint) {
      changed.push({ code: newEntry.code, path: newEntry.path, ignored: newEntry.ignored, baselineChanged: true });
    }
  }
...
function createStartupHandshake(invocationId, socketPath, phase = "materializing") {
  const at = now();
  return {
    invocationId,
    phase,
...
function createStreamAccumulator(onValidEvent) {
  const decoder = new StringDecoder("utf8");
  let pending = "";
  let firstByte = false;
  let finished = false;
  const parsed = { summary: [], terminal: null, invalid: 0, finalText: "" };
  function consumeLine(line) {
    if (!line.trim()) return;
    let event;
    try { event = JSON.parse(line); } catch (_) { parsed.invalid += 1; return; }
    const validation = validateStreamEvent(event);
    if (!validation.valid) { parsed.invalid += 1; return; }
    const type = validation.type;
    if (type === "text") parsed.finalText += event.data;
    parsed.summary.push({
      type,
      sessionId: typeof event.sessionId === "string" ? event.sessionId : null,
      requestId: typeof event.requestId === "string" ? event.requestId : null,
      hasUsage: Boolean(event.usage),
      textBytes: typeof event.data === "string" ? Buffer.byteLength(event.data) : 0
    });
    if (type === "end") parsed.terminal = event;
    onValidEvent(event, validation, parsed);
  }
  return {
    consume(chunk) {
      const buffer = Buffer.isBuffer(chunk) ? chunk : Buffer.from(String(chunk));
      if (buffer.length) firstByte = true;
    ...
    get firstByte() { return firstByte; },
    get parsed() { return parsed; }
  };
}

function executePlanAsync(plan, options = {}) {
  ...
  const terminationConfirmTimeoutMs = Number.isFinite(options.terminationConfirmTimeoutMs)
      && options.terminationConfirmTimeoutMs >= 0
    ? options.terminationConfirmTimeoutMs
    : CHILD_TERMINATION_CONFIRM_TIMEOUT_MS;
  const onProgress = typeof options.onProgress === "function" ? options.onProgress : () => {};
  const spawnWorker = typeof options.spawnWorkerFn === "function"
    ? options.spawnWorkerFn
    : childProcess.spawn;
  const startedMs = Date.now();
  const startedMonotonic = performance.now();
  const startupDeadline = startedMonotonic + startupTimeoutMs;
  const executionDeadline = startedMonotonic + timeoutMs;
  const progressState = options.startupHandshake || createStartupHandshake(plan.invocationId, plan.socket);
  fs.writeFileSync(plan.promptPath, promptFor(plan.capsule), { encoding: "utf8", mode: 0o600 });
  ...
  return new Promise((resolve, reject) => {
    ...
    let custodyUnconfirmedNotified = false;
    const stdoutLimit = MAX_STREAM_BYTES;

    const persist = (patch) => {
      Object.assign(progressState, patch, { lastProgressAt: now() });
      onProgress(clone(progressState));
    };

    const acc = createStreamAccumulator((event, validation) => {
      if (!progressState.firstValidEventAt) {
        const patch = { phase: "first_valid_event", firstValidEventAt: now() };
        if (validation.requestObserved) {
          patch.requestObservation = "observed";
        }
        try { persist(patch); } catch (error) { progressError = error; }
        if (startupTimer) clearTimeout(startupTimer);
      } else if (progressState.requestObservation !== "observed" && validation.requestObserved) {
        try { persist({ requestObservation: "observed" }); } catch (error) { progressError = error; }
      }
    });

    const reasonBeforeFirstEvent = () => {
      if (!progressState.socketObservedAt && !progressState.firstStdoutByteAt) return "child_alive_no_socket_or_stdout";
      if (progressState.socketObservedAt && !progressState.firstValidEventAt) return "socket_present_no_first_valid_event";
      if (progressState.firstStdoutByteAt && !progressState.firstValidEventAt) return "stdout_without_first_valid_event";
      return "first_valid_event_not_observed";
    };
    ...
    const clearTimers = () => {
      ...
      startupTimer = executionTimer = socketTimer = terminationTimer = null;
    };

    const finish = (code, signal, finishOptions = {}) => {
      if (settled) return;
      settled = true;
      clearTimers();
      childClosed = finishOptions.childClosed !== false;
      const parsed = acc.finish();
      const finalFailureClass = failureClass;
      const finalPhase = finalFailureClass && ["STARTUP_HANDSHAKE_BLOCKER", "OS_SPAWN_FAILED"].includes(finalFailureClass)
        ? "startup_failed"
        : "process_exited";
      try {
        persist({
          ...
          termination: childClosed
            ? (progressState.termination === "requested" ? "exited" : "not_required")
            : "unconfirmed",
          cleanup: childClosed ? "pending" : "retained"
        });
      } catch (error) { progressError = progressError || error; }
      if (!childClosed && !custodyUnconfirmedNotified) {
        custodyUnconfirmedNotified = true;
        if (typeof options.onTerminationUnconfirmed === "function") {
          try {
            const transferred = options.onTerminationUnconfirmed(clone(progressState)) === true;
            if (transferred && child) {
              // Custody is now tied to the exact child identity in durable locks;
              // stop holding the CLI process open while that child is recovered.
      ...
      if (progressError) {
        ...
        error.startupHandshake = clone(progressState);
      ...
      resolve({
        ...
        startupHandshake: clone(progressState),
        childClosed,
        childPid: child && Number.isInteger(child.pid) ? child.pid : null,
        childStartTicks: progressState.childStartTicks,
    ...
    const requestTermination = (reasonClass, reason) => {
      if (settled || closed || failureClass) return;
      failureClass = reasonClass;
      failureReason = reason;
      try {
        persist({
          phase: reasonClass === "STARTUP_HANDSHAKE_BLOCKER" || reasonClass === "OS_SPAWN_FAILED"
            ? "startup_failed"
            : "process_exited",
          failureClass,
          failureReason,
          termination: child ? "requested" : "not_required"
        });
      } catch (error) { progressError = progressError || error; }
      if (!child) {
        finish(1, null, { childClosed: true });
        return;
      }
      // Abort covers a ChildProcess that has not emitted `spawn` yet. If a PID
      // is assigned later, the `spawn` handler below also kills that exact child.
      try { spawnAbort.abort(); } catch (_) { /* the ChildProcess handle remains authoritative */ }
      if (!Number.isInteger(child.pid)) {
        terminationTimer = setTimeout(() => {
          if (closed || settled) return;
          try { persist({ termination: "unconfirmed", cleanup: "retained" }); }
          catch (error) { progressError = progressError || error; }
          finish(1, null, { childClosed: false });
        }, terminationConfirmTimeoutMs);
        return;
      }
      try { child.kill(); } catch (_) { /* confirmed by the close event below */ }
      terminationTimer = setTimeout(() => {
        if (closed || settled) return;
        try { child.kill("SIGKILL"); } catch (_) { /* exact child handle only */ }
        terminationTimer = setTimeout(() => {
          if (closed || settled) return;
          try { persist({ termination: "unconfirmed", cleanup: "retained" }); }
          catch (error) { progressError = progressError || error; }
    ...
    const inspectSocket = () => {
      if (settled || progressState.socketObservedAt) return;
      try {
        if (fs.existsSync(plan.socket)) {
          persist({ phase: "socket_observed", socketObservedAt: now() });
          if (progressError && !failureClass) requestTermination("PROVIDER_WAL_UPDATE_FAILED", "startup_progress_persist_failed");
        }
      } catch (error) {
        progressError = progressError || error;
        requestTermination("PROVIDER_WAL_UPDATE_FAILED", "startup_progress_persist_failed");
      }
    };

    try {
      persist({ phase: "spawn_requested", termination: "not_started" });
      child = spawnWorker(plan.executable, plan.args, {
        cwd: plan.cwd,
        env: plan.env,
        windowsHide: true,
        detached: false,
      ...
      failureClass = "OS_SPAWN_FAILED";
      failureReason = "os_spawn_threw_before_child_creation";
      try { persist({ phase: "startup_failed", failureClass, failureReason, termination: "not_required" }); }
      catch (progressFailure) { return reject(progressFailure); }
      return finish(1, null, { childClosed: true });
    }

    const startupRemainingMs = Math.max(0, startupDeadline - performance.now());
    startupTimer = setTimeout(() => {
      if (!progressState.firstValidEventAt) requestTermination("STARTUP_HANDSHAKE_BLOCKER", reasonBeforeFirstEvent());
    }, startupRemainingMs);
    const executionRemainingMs = Math.max(0, executionDeadline - performance.now());
    executionTimer = setTimeout(() => {
      requestTermination(
        progressState.firstValidEventAt ? "WORKER_EXECUTION_TIMEOUT" : "STARTUP_HANDSHAKE_BLOCKER",
        progressState.firstValidEventAt ? "execution_timeout_after_first_valid_event" : reasonBeforeFirstEvent()
      );
    }, executionRemainingMs);
    socketTimer = setInterval(inspectSocket, 25);

    child.once("spawn", () => {
      spawned = true;
      const childPid = Number.isInteger(child.pid) ? child.pid : null;
      const osSpawnedAt = now();
      const childStartTicks = childPid
        ? queryProcessStartTicks(childPid, failureClass ? 1000 : 5000)
        : null;
      try {
        persist({
          phase: "os_spawned",
          ...
          childStartTicks,
          osSpawnedAt,
          requestObservation: progressState.requestObservation === "observed" ? "observed" : "unknown",
      ...
      if (failureClass) {
        ...
        try { child.kill(); } catch (_) { /* exact child handle only */ }
      } else if (performance.now() >= startupDeadline && !progressState.firstValidEventAt) {
        requestTermination("STARTUP_HANDSHAKE_BLOCKER", reasonBeforeFirstEvent());
      }
    });
    ...
    child.stdout.on("data", (chunk) => {
      ...
      if (stdoutBytes > stdoutLimit) {
        requestTermination(
          progressState.firstValidEventAt ? "STREAM_OUTPUT_LIMIT" : "STARTUP_HANDSHAKE_BLOCKER",
          progressState.firstValidEventAt ? "stdout_limit_exceeded" : "stdout_limit_before_first_valid_event"
        );
        return;
      }
      const hadFirstByte = Boolean(progressState.firstStdoutByteAt);
      const priorValidEvent = Boolean(progressState.firstValidEventAt);
      if (!hadFirstByte && bytes > 0) {
        try { persist({ phase: "first_stdout_byte", firstStdoutByteAt: now() }); }
        catch (error) { progressError = progressError || error; }
      }
      acc.consume(chunk);
      if (!priorValidEvent && progressState.firstValidEventAt && startupTimer) clearTimeout(startupTimer);
      inspectSocket();
      if (progressError) requestTermination("PROVIDER_WAL_UPDATE_FAILED", "startup_progress_persist_failed");
      else if (performance.now() >= startupDeadline && !progressState.firstValidEventAt) {
        requestTermination("STARTUP_HANDSHAKE_BLOCKER", reasonBeforeFirstEvent());
      }
    });
    ...
    child.once("error", (error) => {
      errorCode = error && typeof error.code === "string" ? error.code : null;
      requestTermination(spawned ? "WORKER_PROCESS_ERROR" : "OS_SPAWN_FAILED", spawned ? "child_process_error" : "os_spawn_error");
    });
    ...
    child.once("close", (code, signal) => {
      ...
      if (settled) {
        if (!childClosed) {
          const expectedRoot = path.resolve(TEMP_ROOT, plan.invocationId);
          let cleanup = "failed";
          if (path.resolve(plan.invocationRoot) === expectedRoot) {
            try { fs.rmSync(expectedRoot, { recursive: true, force: true }); cleanup = "complete"; }
          ...
          if (typeof options.onLateChildClose === "function") {
            try { options.onLateChildClose(clone(progressState)); }
            catch (error) { progressError = progressError || error; }
          }
        }
        return;
      }
      if (!progressState.firstValidEventAt && !failureClass) {
        failureClass = "STARTUP_HANDSHAKE_BLOCKER";
        failureReason = "child_exited_before_first_valid_event";
      } else if (failureClass) {
        // Keep the original timeout/error classification.
      } else if (code !== 0) {
        failureClass = "WORKER_PROCESS_EXIT";
        failureReason = "child_exited_nonzero";
      }
      if (!progressState.firstValidEventAt && errorCode && errorCode !== "ABORT_ERR"
          && failureClass === "STARTUP_HANDSHAKE_BLOCKER") {
        failureClass = "OS_SPAWN_FAILED";
        failureReason = "os_spawn_error";
      }
      finish(code, signal, { childClosed: true });
    });
...
function buildResultCapsule({ capsule, plan, execution, classification, selectionEvidence, quotaSignal, changedFiles }) {
  ...
  const terminalSucceeded = isSuccessfulTerminalEvent(terminal);
  ...
  if (execution.status === 0 && terminalSucceeded && boundaryOk && !execution.rawCleanupFailed && !hardFailType) {
    finalStatus = "completed";
  }
  const textDigest = crypto.createHash("sha256").update(execution.parsed.finalText || "").digest("hex");
  const workerSummary = boundedWorkerSummary(execution.parsed.finalText || "");
  const stopReason = execution.startupHandshake && execution.startupHandshake.failureClass
    ? execution.startupHandshake.failureClass
    : (classification && classification.errorType === "quota_exhausted" && classification.note !== "no-error-on-success")
    ? "profile_exhausted"
  ...
  return {
    ...
    findings: [
      isObservedTerminalEvent(terminal) ? "terminal end event observed" : "terminal end event missing or invalid",
      ...
      `requestObservation=${requestObservationForExecution(execution)}`,
      ...(execution.startupHandshake ? [
        `startupPhase=${execution.startupHandshake.phase}`,
        `startupFailureClass=${execution.startupHandshake.failureClass || "none"}`,
        `requestObservation=${execution.startupHandshake.requestObservation}`,
        `socketObserved=${Boolean(execution.startupHandshake.socketObservedAt)}`,
        `firstStdoutByteObserved=${Boolean(execution.startupHandshake.firstStdoutByteAt)}`,
        `firstValidEventObserved=${Boolean(execution.startupHandshake.firstValidEventAt)}`
      ] : []),
    ...
    boundaryCompliance: {
      changedFilesFinalState: changes,
      policyAuditEvents: execution.policyAuditEvents || policyAuditEvents,
      allowed: boundaryOk
    },
    ...
    stopReason,
    durationMs: Number.isInteger(execution.durationMs) ? execution.durationMs : null,
...
function persistAvailabilityFromOutcome(profile, classification, executionStatus, context = {}) {
  ...
  const verifiedSuccessfulResult = context.verifiedSuccessfulResult === true
    && executionStatus === 0
    && (!classification || !classification.errorType);
  if (verifiedSuccessfulResult) {
...
function runSingleAttempt(capsule, registry, profile, selectionEvidence, options = {}) {
  ...
  try {
    ...
    changeBaseline = typeof options.changedFilesFinalStateFn === "function"
      ? []
      : (typeof options.changedFilesSnapshotFn === "function"
        ? options.changedFilesSnapshotFn(capsule.worktree.path)
        : changedFilesSnapshot(capsule.worktree.path));
    ...
    changedFiles = typeof options.changedFilesFinalStateFn === "function"
      ? options.changedFilesFinalStateFn(capsule.worktree.path)
      : changedFilesDelta(changeBaseline, changedFilesSnapshot(capsule.worktree.path));
    ...
    const terminalSucceededEarly = isSuccessfulTerminalEvent(terminalEarly);
  ...
  healthOutcome = persistAvailabilityFromOutcome(profile, classification, execution.status, {
    taskId: capsule.taskId,
    invocationId: plan.invocationId,
    verifiedSuccessfulResult: isVerifiedSuccessfulResult(result)
  });
  return {
    result,
    ledger,
...
function requestObservationForExecution(execution) {
  if (execution && execution.startupHandshake
      && ["not_observed", "observed", "unknown"].includes(execution.startupHandshake.requestObservation)) {
    return execution.startupHandshake.requestObservation;
  }
  if (execution && isObservedTerminalEvent(execution.parsed && execution.parsed.terminal)) return "observed";
  if (execution && execution.spawnObserved === false) return "not_observed";
  if (execution && ["ENOENT", "EACCES", "ENOEXEC", "ENOTDIR"].includes(execution.errorCode)) return "not_observed";
  return "unknown";
}

function requestObservationForHandshake(handshake) {
  if (!handshake) return "not_observed";
  if (handshake.requestObservation === "observed") return "observed";
  if (["OS_SPAWN_FAILED", "PROVIDER_PRESPAWN_FAILED"].includes(handshake.failureClass)) return "not_observed";
  if (handshake.phase === "spawn_requested"
      || handshake.osSpawnedAt
      || handshake.childPid !== null
      || handshake.termination === "unconfirmed") return "unknown";
  return "not_observed";
}

function summarizeRequestObservations(observations) {
  const normalized = (observations || []).filter(Boolean);
  if (normalized.includes("unknown")) return { requestObservation: "unknown", realRequests: "unknown" };
  const observed = normalized.filter((item) => item === "observed").length;
  return {
    requestObservation: observed ? "observed" : "not_observed",
    realRequests: observed
  };
}

function attachRequestEvidence(error, evidence) {
  const observations = evidence.attemptObservations.slice();
  if (evidence.currentHandshake) observations.push(requestObservationForHandshake(evidence.currentHandshake));
  const summary = summarizeRequestObservations(observations);
  if (!error.details || !isObject(error.details)) error.details = {};
  error.details.requestObservation = summary.requestObservation;
  error.details.realRequests = summary.realRequests;
  return error;
}

function attachSingleRequestEvidence(error, requestObservation) {
  const summary = summarizeRequestObservations([requestObservation]);
  if (!error.details || !isObject(error.details)) error.details = {};
  error.details.requestObservation = summary.requestObservation;
  error.details.realRequests = summary.realRequests;
  return error;
...
async function runSingleAttemptAsync(capsule, registry, profile, selectionEvidence, options = {}) {
  const profileLock = acquireLock("profile", [profile.grokHome], capsule.worktree.path, 60 * 60 * 1000);
  if (typeof options.onProfileLockAcquired === "function") {
    try { options.onProfileLockAcquired(); } catch (_) { /* selection release best-effort */ }
  }
  ...
  let changeBaseline;
  let changedFiles;
  let startupHandshake = options.startupHandshake || null;
  let retainLocksForChild = false;
  let lockOwnershipTransferred = false;
  const retainLocksForUnconfirmedChild = (handshake) => {
    retainLocksForChild = true;
    const childPid = handshake && handshake.childPid;
    const childStartTicks = handshake && handshake.childStartTicks;
    if (!Number.isInteger(childPid) || typeof childStartTicks !== "string" || !/^\d+$/.test(childStartTicks)) {
      return;
    }
    const changed = [];
    try {
      profileLock.retainForProcess(childPid, childStartTicks);
      changed.push(profileLock);
      if (fileLock) {
        fileLock.retainForProcess(childPid, childStartTicks);
        changed.push(fileLock);
      }
      lockOwnershipTransferred = true;
      return true;
    } catch (error) {
      for (const lock of changed.reverse()) {
        try { lock.restoreToProvider(); } catch (_) { /* keep any un-restored lock fail-closed */ }
      }
      return false;
    }
  };
  const releaseRetainedLocksAfterChildClose = () => {
    if (!retainLocksForChild) return;
    if (fileLock) fileLock.release();
    profileLock.release();
    retainLocksForChild = false;
    lockOwnershipTransferred = false;
    if (typeof options.onLateChildClose === "function") options.onLateChildClose(startupHandshake);
  };
  const persistStartup = typeof options.onStartupProgress === "function"
  ...
  try {
    ...
    try {
      plan = materialize(capsule, registry, {
        ...options,
        profileId: profile.profileId,
        invocationId: startupHandshake && startupHandshake.invocationId,
        skippedReasons: selectionEvidence.skippedReasons || [],
        maintenanceProbePlanned: selectionEvidence.maintenanceProbePlanned === true,
        // Controlled child-process smoke tests may skip only the live `inspect` call.
        skipInspect: options.skipInspect === true || typeof options.spawnWorkerFn === "function"
      });
    } catch (error) {
      if (startupHandshake) {
        const expectedRoot = path.resolve(TEMP_ROOT, startupHandshake.invocationId);
        let cleanup = "complete";
        try { fs.rmSync(expectedRoot, { recursive: true, force: true }); }
        catch (_) { cleanup = "failed"; }
        startupHandshake = {
          ...startupHandshake,
          phase: "startup_failed",
          failureClass: "PROVIDER_PRESPAWN_FAILED",
          failureReason: error && error.code || "materialize_failed",
          termination: "not_required",
          cleanup,
          lastProgressAt: now()
        };
        persistStartup(startupHandshake);
      }
      throw error;
    }
    assert(
      startupHandshake && plan.invocationId === startupHandshake.invocationId
        && path.resolve(plan.socket) === path.resolve(startupHandshake.socketPath),
      "STARTUP_HANDSHAKE_BINDING",
      "The planned invocation does not match its durable startup record."
    );
    let latestHandshake = clone(startupHandshake);
    try {
      execution = await executePlanAsync(plan, {
        timeoutMs: options.timeoutMs,
        startupTimeoutMs: options.startupTimeoutMs,
        terminationConfirmTimeoutMs: options.terminationConfirmTimeoutMs,
        spawnWorkerFn: options.spawnWorkerFn,
        startupHandshake: latestHandshake,
        onTerminationUnconfirmed: retainLocksForUnconfirmedChild,
        onLateChildClose: releaseRetainedLocksAfterChildClose,
        onProgress: (next) => {
          latestHandshake = next;
          startupHandshake = next;
          persistStartup(next);
        }
      });
    } catch (error) {
      startupHandshake = error && error.startupHandshake || startupHandshake;
      if (error && error.childClosed === false) {
        const requestObservation = requestObservationForHandshake(startupHandshake);
        throw attachSingleRequestEvidence(new ProviderError(
          "STARTUP_CHILD_CUSTODY_UNCONFIRMED",
          "The exact worker child did not confirm termination after a Provider progress failure; invocation files are retained and automatic retry is forbidden.",
          {
            invocationId: plan.invocationId,
            childPid: startupHandshake && startupHandshake.childPid,
            childStartTicks: startupHandshake && startupHandshake.childStartTicks,
            lockCustody: lockOwnershipTransferred ? "child" : "provider-retained",
            startupHandshake
          }
        ), requestObservation);
      }
    ...
    startupHandshake = execution.startupHandshake || latestHandshake;
    if (!execution.childClosed) {
      const requestObservation = requestObservationForHandshake(startupHandshake);
      throw attachSingleRequestEvidence(new ProviderError(
        "STARTUP_CHILD_CUSTODY_UNCONFIRMED",
        "The exact worker child did not confirm termination; invocation files are retained and automatic retry is forbidden.",
        {
          invocationId: plan.invocationId,
          childPid: execution.childPid,
          childStartTicks: execution.childStartTicks,
          lockCustody: lockOwnershipTransferred ? "child" : "provider-retained",
          startupHandshake
        }
      ), requestObservation);
    ...
    if (startupHandshake.failureClass && [
      "STARTUP_HANDSHAKE_BLOCKER",
      "OS_SPAWN_FAILED",
      "PROVIDER_WAL_UPDATE_FAILED",
      "PROVIDER_PRESPAWN_FAILED",
      "STREAM_OUTPUT_LIMIT"
    ].includes(startupHandshake.failureClass)) {
      classification = {
        ...
        profileAttributable: false,
        accountLevelEvidence: false,
        note: startupHandshake.failureClass.toLowerCase()
      };
    }
    const parsedEarly = execution.parsed || {};
    const terminalEarly = parsedEarly.terminal || {};
    ...
    if (!retainLocksForChild) {
      if (fileLock) fileLock.release();
      profileLock.release();
    }
  }

  const terminal = execution.parsed && execution.parsed.terminal || {};
  const sessionId = terminal.sessionId || plan.sessionId;
  const requestId = terminal.requestId || `unknown-${plan.invocationId}`;
  const runUsage = numericUsage(terminal.usage);
  if (classification && classification.errorType === "quota_exhausted" && !runUsage.present) {
    runUsage.present = false;
    runUsage.unknown = true;
    runUsage.note = "usage-unknown";
    runUsage.input_tokens = null;
  ...
  const resultClassification = (structuralSuccess && (!classification || !classification.errorType))
    ...
    : normalizedClassification;
  execution.policyAuditEvents = readPolicyAudit(plan.auditPath);

  // A confirmed child exit is the precondition for deleting its invocation root.
  const expectedRoot = path.resolve(TEMP_ROOT, plan.invocationId);
  assert(path.resolve(plan.invocationRoot) === expectedRoot, "INVOCATION_CUSTODY_PATH", "Invocation cleanup target is not the exact Provider-owned root.");
  let cleanupFailed = false;
  try { fs.rmSync(expectedRoot, { recursive: true, force: true }); }
  catch (_) { cleanupFailed = true; }
  execution.rawCleanupFailed = execution.rawCleanupFailed || cleanupFailed;
  if (startupHandshake) {
    startupHandshake = {
      ...startupHandshake,
    ...
    execution.startupHandshake = startupHandshake;
    persistStartup(startupHandshake);
  }

  const result = buildResultCapsule({
    capsule,
    plan,
    execution,
    classification: resultClassification,
    selectionEvidence: { ...selectionEvidence, finalSelectedProfileId: plan.profileId },
    quotaSignal,
    changedFiles
  });
  if (structuralSuccess && (!classification || !classification.errorType)) {
  ...
  return {
    ...
    startupHandshake,
    hasOutput: Boolean(execution.parsed.finalText),
    hasToolEvents: (execution.parsed.summary || []).some((event) => /tool|function/i.test(String(event.type || ""))),
    healthOutcome
  };
}
...
function driveMaybeAsync(iterator) {
  function advance(method, value) {
    let step;
    try { step = iterator[method](value); }
    ...
    return advance("next", yielded);
  }
  return advance("next");
}

function runTask(alias, taskFile, options = {}) {
  const requestEvidence = { attemptObservations: [], currentHandshake: null };
  const runOptions = { ...options, __runRequestEvidence: requestEvidence };
  const annotate = (error) => { throw attachRequestEvidence(error, requestEvidence); };
  let result;
  try { result = driveMaybeAsync(runTaskFlow(alias, taskFile, runOptions)); }
  catch (error) { return annotate(error); }
  return result && typeof result.then === "function" ? result.catch(annotate) : result;
}

function* runTaskFlow(alias, taskFile, options = {}) {
  const registry = loadRegistry();
  ...
  while (currentProfile && !tried.has(currentProfile.profileId)) {
    ...
    let attempt;
    try {
      if (typeof options.executePlanFn === "function") {
        attempt = runSingleAttempt(capsule, registry, currentProfile, attemptEvidence, attemptOptions);
      } else {
        const invocationId = crypto.randomUUID();
        const startupHandshake = createStartupHandshake(
          invocationId,
          path.join(TEMP_ROOT, invocationId, "leader.sock")
        );
        taskRun.startupHandshake = startupHandshake;
        if (options.__runRequestEvidence) options.__runRequestEvidence.currentHandshake = startupHandshake;
        taskRun = availability.writeTaskRun(DATA_ROOT, taskRun, deps);
        attemptOptions.startupHandshake = startupHandshake;
        attemptOptions.onStartupProgress = (next) => {
          taskRun.startupHandshake = next;
          if (options.__runRequestEvidence) options.__runRequestEvidence.currentHandshake = next;
          if (
            taskRun.status === "running"
            && taskRun.takeoverRequired
            && next.termination === "exited"
            && next.cleanup === "complete"
          ) taskRun.status = "interrupted";
          taskRun = availability.writeTaskRun(DATA_ROOT, taskRun, deps);
        };
        attempt = yield runSingleAttemptAsync(capsule, registry, currentProfile, attemptEvidence, attemptOptions);
      }
    } catch (error) {
      if (selectionHeld) {
        selectionLock.release();
      ...
      if (error && error.code === "STARTUP_CHILD_CUSTODY_UNCONFIRMED") {
        taskRun.takeoverRequired = true;
        taskRun.startupHandshake = error.details && error.details.startupHandshake || taskRun.startupHandshake;
        taskRun = availability.writeTaskRun(DATA_ROOT, taskRun, deps);
      } else if (taskRun.status === "running") {
        if (taskRun.startupHandshake) {
          const priorHandshake = taskRun.startupHandshake;
          const childClosedByEvidence = priorHandshake.phase === "process_exited"
            || priorHandshake.termination === "exited"
            || (priorHandshake.childPid === null
              && ["not_started", "not_required"].includes(priorHandshake.termination));
          let cleanup = priorHandshake.cleanup;
          if (childClosedByEvidence && cleanup !== "complete") {
            const expectedRoot = path.resolve(TEMP_ROOT, priorHandshake.invocationId);
            try { fs.rmSync(expectedRoot, { recursive: true, force: true }); cleanup = "complete"; }
            catch (_) { cleanup = "failed"; }
          }
          const startupFailure = [
            "STARTUP_HANDSHAKE_BLOCKER", "OS_SPAWN_FAILED", "PROVIDER_PRESPAWN_FAILED"
          ].includes(priorHandshake.failureClass);
          taskRun.startupHandshake = {
            ...priorHandshake,
            phase: startupFailure || !childClosedByEvidence ? "startup_failed" : "process_exited",
            failureClass: priorHandshake.failureClass || "PROVIDER_RUN_FAILED",
            failureReason: priorHandshake.failureReason || error && error.code || "provider_run_failed",
            termination: childClosedByEvidence
              ? (priorHandshake.termination === "requested" ? "exited" : "not_required")
              : priorHandshake.termination,
            cleanup: cleanup || (childClosedByEvidence ? "complete" : "retained"),
            lastProgressAt: now()
          };
        }
        taskRun.status = "failed";
        taskRun.takeoverRequired = Boolean(
          taskRun.startupHandshake
          && (taskRun.startupHandshake.requestObservation === "observed"
            || taskRun.startupHandshake.requestObservation === "unknown")
        );
        taskRun = availability.writeTaskRun(DATA_ROOT, taskRun, deps);
      }
      throw error;
    ...
    if (options.__runRequestEvidence) {
      options.__runRequestEvidence.attemptObservations.push(attempt.requestObservation || "unknown");
      options.__runRequestEvidence.currentHandshake = null;
    }
    ...
    attempts.push({
      ...
      requestObservation: attempt.requestObservation || requestObservationForExecution({ parsed: { summary: [] } }),
      selectionClass: currentSelectionClass
    });
  ...
  const requestEvidence = summarizeRequestObservations(attempts.map((attempt) => attempt.requestObservation || "unknown"));
  return {
    ...
    attempts,
    probesUsed,
    requestObservations: attempts.map((attempt) => attempt.requestObservation || "unknown"),
    requestObservation: requestEvidence.requestObservation,
    realRequests: requestEvidence.realRequests
  };
}
...
function acquireLock(scope, patterns, root, leaseMs = 30000) {
  ...
  function replaceLockRecord(patch) {
    assert(fs.existsSync(file), "LOCK_CUSTODY_LOST", "The owned lock file disappeared before custody update.", { scope, lockId });
    const current = readJson(file);
    assert(current && current.lockId === lockId, "LOCK_CUSTODY_LOST", "The lock file no longer belongs to this invocation.", { scope, lockId });
    const next = { ...current, ...patch, heartbeatAt: now() };
  ...
  return {
    value,
    heartbeat() { replaceLockRecord({ heartbeatAt: now() }); },
    retainForProcess(pid, processStartTicks) {
      assert(Number.isInteger(pid) && pid > 0 && typeof processStartTicks === "string" && /^\d+$/.test(processStartTicks),
        "LOCK_CUSTODY_IDENTITY", "Retained lock custody requires an exact process identity.", { scope });
      return replaceLockRecord({
        pid,
        processStart: 0,
        processStartTicks,
        leaseMs: Number.MAX_SAFE_INTEGER,
        custodyOwner: "worker-child"
      });
    },
    restoreToProvider() {
      return replaceLockRecord({ ...originalOwner, custodyOwner: "provider" });
    },
    release() {
      if (!fs.existsSync(file)) return;
      let current;
      try { current = readJson(file); } catch (_) { return; }
      if (current && current.lockId === lockId) fs.rmSync(file, { force: true });
    }
  };
}
...
function applyMaintenanceProbeOutcome(record, classification, execution, options = {}) {
  ...
  if (exitCode === 0 && end && sessionId && requestId && expectedMatched) {
    return {
      outcome: "recovered",
...
function executeMaintenanceProbe(profile, options = {}) {
  ...
  const terminalValid = isObservedTerminalEvent(terminal);
  const end = Boolean(terminalValid);
  const terminalSessionId = terminalValid ? String(terminal.sessionId) : null;
  const requestId = terminalValid ? String(terminal.requestId) : null;
  ...
  if (execution.status === 0 && expectedResponseMatched && end && sessionId && requestId) {
    classification = {
    ...
    for (const candidate of selection.selected) {
      ...
      try {
        ...
        const outcome = applyMaintenanceProbeOutcome(postRecord, probe.classification, probe.execution, {
          ...
          sessionId: probe.terminalSessionId,
...
function validateResultCapsule(result) {
  ...
  if (Object.prototype.hasOwnProperty.call(result, "workerSummary")) {
    assert(typeof result.workerSummary === "string" && Buffer.byteLength(result.workerSummary, "utf8") <= MAX_WORKER_SUMMARY_BYTES + 32, "RESULT_SUMMARY", "workerSummary is invalid or too large.");
  }
...
async function main(argv) {
  ...
  // Idempotent ensureDir only for all paths; mutation side-effects skipped on read-only.
  ensureDir(DATA_ROOT);
  if (!readOnly) {
    ensureDir(TEMP_ROOT);
  ...
  try {
    ...
    else if (command === "run") output = await runTask(args.profile, args.task, { timeoutMs: args.timeout ? Number(args.timeout) : undefined });
    ...
    if (error && error.name === "ProviderError") {
      const details = sanitizeErrorDetails(isObject(error.details) ? error.details : {});
      process.stdout.write(`${JSON.stringify({
        error: error.code,
        message: error.safeMessage,
        details,
        requestObservation: ["observed", "not_observed", "unknown"].includes(details.requestObservation)
          ? details.requestObservation
          : "unknown",
        realRequests: Object.prototype.hasOwnProperty.call(details, "realRequests")
          ? details.realRequests
          : "unknown"
      }, null, 2)}\n`);
      process.exitCode = error.exitCode || 1;
      return;
    }
    const details = sanitizeErrorDetails(isObject(error && error.details) ? error.details : {});
    process.stdout.write(`${JSON.stringify({
      error: error && error.code || "PROVIDER_UNEXPECTED_ERROR",
      message: redactText(error && error.message || "grok-worker failed"),
      details,
      requestObservation: ["observed", "not_observed", "unknown"].includes(details.requestObservation)
        ? details.requestObservation
        : "unknown",
...
module.exports = {
  ...
  availability,
  _test: {
    atomicWriteJson, redactText, boundedWorkerSummary, readJson, hasSecretKeys, emptyLedger, usageFingerprint,
    isolatedEnv, readTrustStore, inspectEffective, inspectAuthorityFindings, projectSettingsHasAuthority,
    isProviderInjectedPermissionSource, spawnCapture, availabilityDeps,
    changedFilesSnapshot, changedFilesDelta, pathContentFingerprint,
    queryProcessStartTicks, captureRunOwner, inspectRunOwner, taskRunTransaction,
