...
1. 耗尽**分类语义**已确认：`status_code=402` + `is_retryable=false` + `Grok Build usage balance exhausted`（账号 ayun0300）。
...
5. `grok models` 能过 ≠ 账号可用（四账号都过但 ayun0300 仍 402）→ 解封只认最小真实请求成功。
...
- **唯一权威源码仓库**：`D:\Grok Worker Provider`
...
- `D:\Grok UI\.codex\grok-bridge\provider`：只读迁移源/历史镜像，**不再是权威源码**。
...
- 已确认 402 的 ayun0300 → `frozen/quota/resetAt:null`（依 §12 受控历史证据做一次显式 bootstrap）。
